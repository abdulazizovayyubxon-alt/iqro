...
const TARGET_DIR = path.join(__dirname, 'fan', 'informatika');
const REPORT_FILE = path.join(__dirname, 'fan', 'info_report.md');
...
async function main() {
  console.log("🔍 Scanning Informatika va AT question blocks...");
  ...
  files.forEach(file => {
    ...
    data.questions.forEach((q, idx) => {
      ...
      if (!q.subject || q.subject !== "Informatika va axborot texnologiyalari") {
        schemaErrors.push(`${ref}: Invalid or missing "subject" (must be "Informatika va axborot texnologiyalari"). Got: ${q.subject}`);
      }
  ...
  const reportMd = `# Informatika va AT Question Bank Audit Report
