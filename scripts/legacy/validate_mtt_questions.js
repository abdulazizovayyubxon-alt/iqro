...
const TARGET_DIR = path.join(__dirname, 'fan', 'mtt_tarbiyachi');
const REPORT_FILE = path.join(__dirname, 'fan', 'mtt_report.md');
...
async function main() {
  console.log("🔍 Scanning MTT Tarbiyachi question blocks...");
  ...
  files.forEach(file => {
    ...
    data.questions.forEach((q, idx) => {
      ...
      if (!q.subject || q.subject !== "MTT tarbiyachi") schemaErrors.push(`${ref}: Invalid or missing "subject" (must be "MTT tarbiyachi").`);
  ...
  const reportMd = `# MTT Tarbiyachi Question Bank Audit Report
