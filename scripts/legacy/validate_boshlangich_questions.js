/**
 * validate_boshlangich_questions.js
 * Scans, validates, and audits all generated questions under fan/boshlangich/ directory.
 * Generates a final markdown report with statistics.
 */
import fs from 'fs';
...
const TARGET_DIR = path.join(__dirname, 'fan', 'boshlangich');
const REPORT_FILE = path.join(__dirname, 'fan', 'boshlangich_report.md');
...
async function main() {
  console.log("🔍 Scanning question blocks...");
  ...
  let schemaErrors = [];
  let duplicates = [];
  const questionTexts = new Map(); // text -> file & id
  ...
  files.forEach(file => {
    ...
    data.questions.forEach((q, idx) => {
      ...
      if (!q.subject || q.subject !== "Boshlang‘ich ta’lim") schemaErrors.push(`${ref}: Invalid or missing "subject" (must be "Boshlang‘ich ta’lim").`);
  ...
  const reportMd = `# Boshlang'ich Ta'lim Question Bank Audit Report
...
* **Schema Schema Violations:** ${schemaErrors.length}
...
## Topic Distribution
${Object.entries(topicStats).map(([topic, cnt]) => `* **${topic}:** ${cnt}`).join('\n')}

${schemaErrors.length > 0 ? `\n## Schema Errors\n${schemaErrors.map(e => `* ❌ ${e}`).join('\n')}` : '\n## Schema Status\n* ✅ All JSON schemas are perfectly valid.'}
