...
const TARGET_DIR = nodePath.join(__dirname, 'fan', 'chqbt');
const REPORT_FILE = nodePath.join(__dirname, 'fan', 'chqbt_report.md');
...
async function main() {
  console.log("🔍 Scanning CHQBT question blocks...");
  ...
  files.forEach(file => {
    ...
    data.questions.forEach((q, idx) => {
      ...
      if (!q.subject || q.subject !== "Chaqiruvga qadar boshlang'ich tayyorgarlik (CHQBT)") {
  ...
  const reportMd = `# CHQBT Question Bank Audit Report
