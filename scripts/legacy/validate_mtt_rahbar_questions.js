...
const TARGET_DIR = path.join(__dirname, 'fan', 'mtt_rahbar');
const REPORT_FILE = path.join(__dirname, 'fan', 'mtt_rahbar_report.md');
...
async function main() {
  console.log("🔍 Scanning MTT Direktor O'rinbosari question blocks...");
  ...
  files.forEach(file => {
    ...
    data.questions.forEach((q, idx) => {
      ...
      if (!q.subject || q.subject !== "MTT direktor o'rinbosari") schemaErrors.push(`${ref}: Invalid or missing "subject" (must be "MTT direktor o'rinbosari").`);
  ...
  const reportMd = `# MTT Direktor O'rinbosari Question Bank Audit Report
