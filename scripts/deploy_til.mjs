...
async function main() {
  console.log('--- ONA TILI VA ADABIYOT (TIL) SMART BUNDLE YUKLASH BOSHLANDI ---');

  // 1. Read local cleaned questions
  const localQuestionsPath = 'src/data/questions_til.json';
  ...
  const list = rawQuestions.map((q, idx) => ({
    id: q.id || `til_${String(idx + 1).padStart(4, '0')}`,
    q: q.q || q.question,
  ...
  // 3. Upload Firestore Bundle Chunks (questionBundles/til__n)
  ...
  console.log(`   - fsBundles.til: ${chunks.length} ta bo'lak, ${list.length} ta savol`);
  ...
  console.log(`\n🎉 BARCHASI TAYYOR! Ona tili va adabiyot to'liq paketi bulutga yuklandi.`);
  process.exit(0);
}
