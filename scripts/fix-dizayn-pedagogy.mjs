...
const ovFile = 'fan 4/_app/overrides/texnologiya_dizayn.json';
const overrides = fs.existsSync(ovFile) ? JSON.parse(fs.readFileSync(ovFile, 'utf8')) : {};
const dizayn = JSON.parse(fs.readFileSync('fan 4/_app/texnologiya_dizayn.json', 'utf8'));
...
dizayn.forEach(q => {
  const srcId = String(parseInt(q.docId.replace('texnologiya_dizayn_', ''), 10));
...
console.log(`✅ Dizayn overrides saqlandi: ${fixed} ta savol tuzatildi.`);
