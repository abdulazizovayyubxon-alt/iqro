...
const questions = JSON.parse(fs.readFileSync('fan 4/_app/tarbiya.json', 'utf8'));
...
console.log('\n--- Trap Questions (First 5) ---');
trapQuestions.slice(0, 5).forEach(x => {
  console.log(`ID: ${x.id}, Q: "${x.q.slice(0, 70)}"`);
  x.opts.forEach((o, i) => console.log(`  [${i}] ${o}`));
});

console.log('\n--- High Ratio (First 5) ---');
highRatio.slice(0, 5).forEach(x => {
