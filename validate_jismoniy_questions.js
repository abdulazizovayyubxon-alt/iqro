...
const TARGET_DIR = path.join(__dirname, 'fan', 'jismoniy_tarbiya');
const REPORT_FILE = path.join(__dirname, 'fan', 'jismoniy_report.md');
...
async function main() {
  console.log("🔍 Scanning Jismoniy Tarbiya question blocks...");
  ...
  files.forEach(file => {
    ...
    data.questions.forEach((q, idx) => {
      ...
      if (!q.subject || q.subject !== "Jismoniy tarbiya") schemaErrors.push(`${ref}: Invalid or missing "subject" (must be "Jismoniy tarbiya").`);
  ...
  const reportMd = `# Jismoniy Tarbiya Question Bank Audit Report
